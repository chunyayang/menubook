<template>
  <div class="store-list">
    ...
    <ul v-else class="d-flex flex-column flex-sm-row list">
      <li v-for="store in stores" :key="store.id" class="flex sm6 px-sm-2">
        <v-card flat>
          <v-img :src="require('../assets/' + store.imageUrl)" aspect-ratio="2"></v-img>
          ...
          <v-card-text>
            <div>
              <span class="pr-1">
                <v-icon color="orange" small>{{ svgStar }}</v-icon>
              </span>
